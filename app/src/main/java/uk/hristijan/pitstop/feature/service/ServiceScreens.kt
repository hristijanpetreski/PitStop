...
import uk.hristijan.pitstop.core.format.formatMoney as formatMoneyDisplay
        ...
        items(services, key = { it.service.id }) { value ->
            ElevatedCard(onClick = { onServiceSelected(value.service.id) }, modifier = Modifier.fillMaxWidth()) {
                Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    ...
                    Text(formatMoneyDisplay(value.service.totalCostMinor, currencyCode), style = MaterialTheme.typography.titleSmall)
                }
            }
        }
...
fun ServiceFormContent(
    ...
    Scaffold(
      ...
      LazyColumn(
        ...
        item { NumericField(state.laborCost, { value -> update { it.copy(laborCost = value) } }, "Labor cost", currencyCode, state.errors["labor"], true) }
        item {
    ...
    Scaffold(
      ...
      LazyColumn(Modifier.fillMaxSize().padding(innerPadding).padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        items(value.items, key = { it.id }) { item ->
            DetailRow("${item.description} × ${item.quantity}", formatMoneyDisplay(item.unitCostMinor * item.quantity, currencyCode))
        }
        item { HorizontalDivider() }
        item { DetailRow("Parts", formatMoneyDisplay(value.items.sumOf { it.unitCostMinor * it.quantity }, currencyCode)) }
        item { DetailRow("Labor", formatMoneyDisplay(service.laborCostMinor, currencyCode)) }
        item { DetailRow("Total", formatMoneyDisplay(service.totalCostMinor, currencyCode), true) }
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                FormTextField(item.unitPrice, { update { row -> row.copy(unitPrice = it) } }, "Unit price ($currencyCode)", errors["item.$index.price"], KeyboardType.Decimal, Modifier.weight(1f))
            }
        }
    }
}
...
@Composable private fun SummaryRow(label: String, minor: Long) {
    val currencyCode = LocalCurrency.current
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
        Text(label)
        Text(formatMoneyDisplay(minor, currencyCode))
    }
}
